...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, transform=None):
        ...
        # target_sino_path=sorted(glob(os.path.join(saved_path, '*_target_sino.npy')))
        ...
        if mode == 'train':
            ...
            # target_sino=[f for f in target_sino_path if test_patient not in f]
            if load_mode == 0: # batch data load
                self.input_ = input_
                self.target_ = target_
                # self.target_sino = target_sino
            else: # all data load
            ...
            # target_sino = [f for f in target_sino_path if test_patient in f]
            if load_mode == 0:
                self.input_ = input_
                self.target_ = target_
                # self.target_sino = target_sino
    ...
    def __getitem__(self, idx):
        ...
        # else:
        return (np.array(input_img),np.array(target_img))
...
def get_loader(mode='train', load_mode=0,
               ...
               transform=None, batch_size=32, num_workers=1):
    dataset_ = ct_dataset(mode, load_mode, saved_path, test_patient,transform)
