...
def sample_img_to_groups(dl,num, divisor,device,results_folder,q_sample,num_timesteps):
    ...
    for i in range(0,groups):
        ...
        data=data.to(device)
        cond_n = normalize_to_neg_one_to_one(cond)
...
class GaussianDiffusionImg(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert not (type(self) == GaussianDiffusionImg and model.channels != model.out_dim)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(len(torch.unique(img)),img.max())
        # return img
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        val_dl = DataLoader(self.val_ds, batch_size=train_batch_size, shuffle=True, pin_memory=True, num_workers=0)
    ...
    def test_form_data(self,x_path):
        ...
        for idx in range(1,len(self.keys)):
            cond_path = str(x_path).replace(self.keys[0],self.keys[idx])
            cond_img = Image.open(cond_path)
            cond_imgs.append(self.val_ds.transform(cond_img))

        cond_img_t=torch.cat(cond_imgs,dim=0).unsqueeze(0).to(device)
        x_img = self.val_ds.transform(Image.open(x_path)).unsqueeze(0).to(device)
        ...
        imageio.imsave(os.path.join(results_folder_ouput,file_name),reuslt_img)
        imageio.imsave(os.path.join(results_folder_target, file_name), x_img)
        ...
        if self.result_temp!=None:
            ...
            imageio.mimsave( os.path.join(results_folder_ouput,file_name.replace("png","gif")), frames_t)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    data,cond = next(self.dl)#.to(device)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        with torch.no_grad():
                            ...
                            all_images_list = list(map(lambda n,c: self.ema.ema_model.sample(img=n,cimg=c,result_temp=self.result_temp), smaple_imgs,conda_imgs))
